...
<head>
    <meta charset="utf-8" />
    <title>Account Verification 2 - Smart Force</title>
            ...
            <div class="row justify-content-center">
                <div class="col-lg-5 col-md-8">
                    ...
                    <div class="card login-page bg-white shadow rounded border-0">
                        @if ($isVerify == true)
                        <div class="card-body">
                            <h5 class="fs-2 text-center">Verification Success</h5>
                            <p class="text-center">Thank you creating an <strong>SmartForce</strong> account.</p>
                            <p class="text-center">
                                <!-- <strong>Please wait...</strong> -->
                            </p>
                            <form action="{{route('verifyMe')}}" method="POST">
                                @csrf
                                <input type="text" hidden name="email" value="{{$email}}">
                                <input type="text" hidden name="token" value="{{$token}}">
                                <button class="w-100 btn btn-outline-primary">proceed</button>
                            </form>
                        </div>
                        @else
                        <div class="card-body">
                            <h5 class="fs-2 text-danger text-center">Verification Failed</h5>
                            <p>We are unable to verify your email at the moment.</p>
                            <p>
                                <a href="/register">Click here</a> <span>to try again</span>
                            </p>
                        </div>
                        @endif
                    </div>
